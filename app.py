...
def audio_callback(frame: av.AudioFrame) -> av.AudioFrame:
    ...
    try:
        completion = groq_client.chat.completions.create(
            ...
            messages=[
                ...
                {"role": "user", "content":[{"type":"text","text": therapist_response}]}
            ],
            temperature=0.7,
        ...
        patient_response = "Sorry, there was an error processing your input."

    # Convert Groq response to TTS and play in browser
    ...
    st.session_state['latest_audio'] = tts_buffer.getvalue()
    st.session_state['latest_text'] = patient_response
...
try:
    ...
    while not tts_queue.empty():
        latest_bytes = tts_queue.get_nowait()

    if latest_bytes is not None:
        st.audio(BytesIO(latest_bytes), format='audio/wav')
except Exception:
