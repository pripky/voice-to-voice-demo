...
st.session_state["audio_input"] = st.audio_recorder(
...
if st.session_state["audio_input"] is not None:
    # Convert uploaded audio bytes to BytesIO
    audio_bytes = BytesIO(st.session_state["audio_input"])
    st.audio(audio_bytes, format="audio/wav")  # optional: play what user recorded

    # Call your existing audio_callback logic, but pass audio_bytes
